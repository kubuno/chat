...
/// A column's stored byte size, per engine. On PostgreSQL this is
/// `pg_column_size()` (the TOASTed/compressed size the instance actually holds);
/// MySQL/SQLite have no equivalent, so the logical `LENGTH()` is used — a small
/// over-statement of base64url text, accepted for portability.
fn bytes_of(backend: Backend, col: &str) -> String {
    match backend {
        Backend::Postgres => format!("pg_column_size({col})"),
        Backend::MySql | Backend::Sqlite => format!("LENGTH({col})"),
    }
}

/// `COALESCE(SUM(expr), 0)` decodable as `i64`, in the engine's spelling. The
/// dialect's `sum_bigint` takes a `&'static str`, but `expr` here is built at
/// run time from [`bytes_of`], so the cast is spelled directly.
fn sum_bigint(backend: Backend, expr: &str) -> String {
    let inner = format!("COALESCE(SUM({expr}), 0)");
    match backend {
        Backend::Postgres => format!("({inner})::bigint"),
        Backend::MySql => format!("CAST({inner} AS SIGNED)"),
        Backend::Sqlite => format!("CAST({inner} AS INTEGER)"),
    }
}

/// Every byte-bearing query chat runs, paired with the category it feeds, built
/// for the pool's engine.
///
/// Each statement returns exactly `(owner uuid, bytes bigint, objects bigint)`
/// and reads only chat's own schema. Keeping them in one function rather than
/// scattered through the code is what lets the tests below assert, mechanically,
...
fn owned_queries(backend: Backend) -> Vec<(&'static str, String)> {
    ...
    vec![
        ...
        // exists once. `media_meta` travels with the message and is counted with
        // it. Soft-deleted messages keep only a tombstone whose `encrypted_data`
        // the delete path already emptied, so they cost what they actually weigh.
        (
            CAT_CONTENT,
            format!(
                "SELECT sender_id,
                ...
                sum = sum_bigint(
                    ...
                    &format!(
                        "{} + COALESCE({}, 0)",
                        bytes_of(backend, "encrypted_data"),
                        bytes_of(backend, "media_meta")
                    )
                )
            ),
        ),
        // Media blobs live on the storage backend, so the size comes from the
        // column the upload path wrote, not from a byte-size function. A row whose
        ...
        (
            ...
            format!(
                ...
                sum = sum_bigint(backend, "COALESCE(encrypted_size, 0)")
            ),
        ),
        ...
        (
            ...
            format!(
                ...
                sum = sum_bigint(
                    ...
                    &format!(
                        "{} + {}",
                        bytes_of(backend, "identity_key_pub"),
                        bytes_of(backend, "fingerprint")
                    )
                )
            ),
        ),
        (
            ...
            format!(
                ...
                sum = sum_bigint(
                    ...
                    &format!(
                        "{} + {}",
                        bytes_of(backend, "public_key"),
                        bytes_of(backend, "signature")
                    )
                )
            ),
        ),
        (
            ...
            format!(
                "SELECT user_id,
                  ...
                  GROUP BY user_id",
                sum = sum_bigint(backend, &bytes_of(backend, "public_key"))
            ),
        ),
        // A sender key is generated by `sender_id` for each recipient. It is the
        // sender's machinery, so the sender carries it — the same attribution as
        // the messages it protects.
        (
            CAT_SYSTEM,
            format!(
                "SELECT sender_id,
                  ...
                  GROUP BY sender_id",
                sum = sum_bigint(backend, &bytes_of(backend, "encrypted_key"))
            ),
        ),
        (
            CAT_SYSTEM,
            format!(
                ...
                sum = sum_bigint(
                    ...
                    &format!(
                        "{} + {} + COALESCE({}, 0)",
                        bytes_of(backend, "device_name"),
                        bytes_of(backend, "identity_key_pub"),
                        bytes_of(backend, "push_token")
                    )
                )
            ),
        ),
...
mod tests {
    ...
    fn nothing_is_ever_charged_to_a_recipient() {
        for (_, sql) in owned_queries(Backend::Sqlite) {
